...
<ol class="breadcrumb">
    <li>Edit</li>
    <li><a href="{{ route('post.view', ['title_url' => $post->title_url ]) }}">{{ $post->title }}</a></li>
</ol>
<hr>
<form class="form-vertical form-submit-normal" action="{{ route('admin.post.edit', ['post_id' => $post->id]) }}" method="post" enctype="multipart/form-data">
    <div class="form-group  {{ $errors->has('title') ? 'has-error' : '' }}">
        <label for="title" class="control-label">Title</label>
        <input required="required" value="{{ Request::old('title') ?: $post->title }}" name="title" type="text" class="form-control" id="title">
        @if($errors->has('title'))
    ...
    <div class="form-group  {{ $errors->has('title_url') ? 'has-error' : '' }}">
        ...
        <input required="required" value="{{ Request::old('title_url') ?: $post->title_url }}" name="title_url" type="text" class="form-control" id="title_url">
        ...
        <div class="row">
            <div class="col-xs-6 col-md-3">
                <a href="#" data-toggle="modal" data-target="#file-manager">
                    <img id="post-image" src="{{ $file_url ?: $post->files()->first()->thumbnail_url() }}" alt="Upload image" style="height:100px;" />
                </a>
            </div>
        </div>
        <input value="{{ Request::old('file_id') ?: $post->files()->first()->id }}" name="file_id" type="hidden" class="form-control" id="post-image-id">
    ...
    <div class="form-group  {{ $errors->has('keywords') ? 'has-error' : '' }}">
        ...
        <input value="{{ Request::old('keywords') ?: $post->keywords }}" name="keywords" type="text" class="form-control" id="keywords">
    ...
    <div class="form-group  {{ $errors->has('description') ? 'has-error' : '' }}">
        ...
        <textarea name="description" rows="2" class='form-control' id="description">{{ Request::old('description') ?: $post->description }}</textarea>
    ...
    <div class="form-group  {{ $errors->has('body') ? 'has-error' : '' }}">
        ...
        <textarea id="body" name='body'>{{ Request::old('body') ?: $post->body }}</textarea>
        @if($errors->has('body'))
    ...
    <button type="submit" class='btn btn-lg btn-default'>Update & View</button>
